...
while ( have_posts() ) :
	...
	$anh_ve_chung_toi = function_exists( 'eyecare_anh_giao_dien_lay_nhieu' )
		? eyecare_anh_giao_dien_lay_nhieu( array( 've_chung_toi_1', 've_chung_toi_2', 've_chung_toi_3' ), 'large' )
		: array();
	...
	<article id="trang-<?php the_ID(); ?>" <?php post_class( 'eyecare-gioi-thieu eyecare-ve-chung-toi' ); ?>>
		<section class="eyecare-page-hero eyecare-page-hero--gioi-thieu eyecare-page-hero--ve-chung-toi" aria-labelledby="gioi-thieu-tieu-de">
		...
		<p class="eyecare-page-hero__nhan">Về chúng tôi</p>
		<h1 id="gioi-thieu-tieu-de">Bệnh viện Mắt Hà Nội – Bắc Ninh</h1>
		<p class="eyecare-page-hero__dan"><?php echo esc_html( $tt['ten'] ); ?> là địa chỉ chuyên khoa Mắt phục vụ người dân tại Bắc Ninh, Bắc Giang và khu vực lân cận. Trang này giúp bạn hiểu nhanh về bệnh viện, thế mạnh chuyên môn, đội ngũ và cách chuẩn bị trước khi đến.</p>
						<div class="eyecare-gioi-thieu__hero-actions">
						<a class="eyecare-gioi-thieu__nut eyecare-gioi-thieu__nut--vang" href="#tong-quan-benh-vien">Xem tổng quan <span aria-hidden="true">↓</span></a>
					...
					<div class="eyecare-gioi-thieu__hero-visual" aria-label="Đại diện đội ngũ bác sĩ chuyên khoa Mắt">
						...
						<div class="eyecare-gioi-thieu__team-note">
							...
							<small>Hồ sơ đồng bộ từ trang quản trị</small>
						</div>
					</div>
				</div>
		...
		<section id="tong-quan-benh-vien" class="eyecare-gioi-thieu__tong-quan" aria-labelledby="tong-quan-tieu-de">
			<div class="eyecare-noi-dung__khung eyecare-gioi-thieu__tong-quan-grid">
				<div class="eyecare-gioi-thieu__tong-quan-copy">
					...
					<h2 id="tong-quan-tieu-de">Một bệnh viện mắt được tổ chức quanh sự rõ ràng và thuận tiện</h2>
					<p>Bệnh viện tập trung vào chuyên khoa Mắt, từ thăm khám ban đầu, theo dõi thị lực đến hướng dẫn chuẩn bị cho các chỉ định chuyên môn. Hệ thống nội dung trên website được sắp xếp theo chuyên khoa, dịch vụ, đội ngũ bác sĩ, hỏi đáp và kiến thức nhãn khoa để người đọc tìm đúng nội dung nhanh hơn.</p>
				...
				<div class="eyecare-gioi-thieu__tong-quan-side">
					<?php if ( $anh_ve_chung_toi ) : ?>
						<div class="eyecare-gioi-thieu__facility" aria-label="Hình ảnh cơ sở vật chất được quản lý từ admin">
							<?php foreach ( $anh_ve_chung_toi as $i => $anh ) : ?>
								<figure class="eyecare-gioi-thieu__facility-item eyecare-gioi-thieu__facility-item--<?php echo esc_attr( (string) ( $i + 1 ) ); ?>">
									<img src="<?php echo esc_url( $anh['url'] ); ?>" alt="<?php echo esc_attr( $anh['alt'] ); ?>" width="<?php echo esc_attr( (string) $anh['width'] ); ?>" height="<?php echo esc_attr( (string) $anh['height'] ); ?>" loading="lazy" decoding="async">
								</figure>
							<?php endforeach; ?>
							<span class="eyecare-gioi-thieu__facility-badge">Ảnh cập nhật từ admin</span>
						</div>
					<?php endif; ?>
		...
		<?php if ( $doi_ngu ) : ?>
			<section class="eyecare-gioi-thieu__doi-ngu" aria-labelledby="gioi-thieu-doi-ngu-tieu-de">
				<div class="eyecare-noi-dung__khung eyecare-gioi-thieu__doi-ngu-grid">
					<header>
						<p class="eyecare-gioi-thieu__eyebrow">Con người</p>
						<h2 id="gioi-thieu-doi-ngu-tieu-de">Đội ngũ bác sĩ chuyên khoa Mắt</h2>
						<span>Danh sách, chức danh và hình ảnh được lấy trực tiếp từ khu vực quản lý bác sĩ trong WordPress.</span>
						<a href="<?php echo esc_url( home_url( '/doi-ngu-bac-si/' ) ); ?>">Xem hồ sơ đội ngũ <span aria-hidden="true">→</span></a>
					</header>
					<div class="eyecare-gioi-thieu__doctor-list">
						<?php foreach ( $doi_ngu as $bac_si ) :
							$ten_bac_si = function_exists( 'eyecare_doi_ngu_ten_day_du' ) ? eyecare_doi_ngu_ten_day_du( $bac_si ) : trim( ( $bac_si['hoc_vi'] ?? '' ) . ' ' . ( $bac_si['ho_ten'] ?? '' ) );
							?>
							<div class="eyecare-gioi-thieu__doctor-row">
								<div class="eyecare-gioi-thieu__doctor-photo">
									<?php if ( ! empty( $bac_si['anh'] ) && wp_get_attachment_image_src( (int) $bac_si['anh'] ) ) : ?>
										<?php echo wp_get_attachment_image( (int) $bac_si['anh'], 'thumbnail', false, array( 'alt' => $ten_bac_si, 'loading' => 'lazy', 'decoding' => 'async' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
									<?php elseif ( ! empty( $bac_si['anh_url'] ) ) : ?>
										<img src="<?php echo esc_url( $bac_si['anh_url'] ); ?>" alt="<?php echo esc_attr( $ten_bac_si ); ?>" loading="lazy" decoding="async">
									<?php endif; ?>
								</div>
								<div><strong><?php echo esc_html( $ten_bac_si ); ?></strong><span><?php echo esc_html( $bac_si['chuc_danh'] ?? 'Bác sĩ chuyên khoa Mắt' ); ?></span></div>
								<?php if ( ! empty( $bac_si['badge'] ) ) : ?><small><?php echo esc_html( $bac_si['badge'] ); ?></small><?php endif; ?>
							</div>
						<?php endforeach; ?>
					</div>
				</div>
