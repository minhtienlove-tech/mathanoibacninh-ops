...
Trạng thái: **CHƯA TRIỂN KHAI PRODUCTION. Chờ người dùng/bác sĩ duyệt.**

Xem trên máy đang chạy Codex: http://127.0.0.1:8783/kien-thuc/#cau-tao-mat-3d
Địa chỉ này chỉ hoạt động trên máy này khi tiến trình preview còn chạy; không phải URL công khai để gửi sang điện thoại khác.
...
- Preview dùng bản HTML công khai của trang hiện tại cộng đoạn nhúng do PHP mới render. Kiểm tra loại bỏ phần chèn trả lại HTML ban đầu byte-for-byte. Đây là xem thử giao diện, chưa phải một bản WordPress staging có database thật.
...
- Console mô hình chạy bình thường có cảnh báo bản Three.js UMD cũ, giữ thư viện gốc theo yêu cầu. Trong preview có ghi nhận lỗi MutationObserver không kèm URL/stack; listener lỗi đặt vào tài liệu không bắt được lỗi này, chưa xác định nguồn (có thể thuộc lớp công cụ trình duyệt). Không gán lỗi này cho mã mới khi chưa có bằng chứng. Các chức năng và cầu nối chiều cao vẫn chạy. Cần kiểm tra lại console trong trình duyệt thông thường khi duyệt/staging.
- `scripts/verify-live.ps1 -CheckSsh`: 6 URL production trả 200. SHA256 hai file production functions.php/page-kien-thuc.php khớp bản trước công việc; chưa upload thay đổi.
...
## Triển khai sau khi được duyệt và khôi phục

1. Chỉ thực hiện sau xác nhận duyệt bản này. Pull, đối chiếu thay đổi mới và bảo đảm không có máy khác deploy.
2. Dùng khóa deploy theo `docs/DEPLOYMENT.md`. Backup database cùng functions.php/page-kien-thuc.php và các file đích đã tồn tại vào thư mục có timestamp dưới `/home/jwhxtzru/backups/`.
3. Upload module và assets mới trước, sau đó functions.php/page-kien-thuc.php. Không upload `.tmp` hoặc tài liệu HTML độc lập gốc. Không thay đổi bài viết hay SEO trong database.
4. PHP lint trên server, kiểm tra hash, purge LiteSpeed, smoke test và thử trực tiếp URL thật trên desktop/mobile. Ghi trạng thái vào CHANGELOG.
5. Khôi phục bằng bản backup functions.php và page-kien-thuc.php của chính lần deploy, rồi purge cache, lint và smoke test. File assets/module mới có thể để nguyên khi không còn được nạp; không cần xóa. Nếu có thay đổi khác sau triển khai, hoàn tác riêng dòng include/render thay vì chép đè các sửa đổi mới.
6. Không tự động restore database: thay đổi này không ghi database và restore có thể làm mất lịch khám phát sinh. Bản database backup chỉ dùng khi cần, có xác nhận riêng.

Hiện tại production chưa đổi nên không cần khôi phục production. Có thể đóng tab xem thử; nếu cần dừng server, kiểm tra PID trong `server.pid` đúng là Node chạy `server.cjs` trước khi dừng.
